...
TFFaceMesh.prototype.getEyePatches = async function (
  ...
  const [leftBBox, rightBBox] = [
    // left
    {
      eyeTopArcKeypoints: [25, 33, 246, 161, 160, 159, 158, 157, 173, 243],
      eyeBottomArcKeypoints: [25, 110, 24, 23, 22, 26, 112, 243],
    },
    // right
    {
      eyeTopArcKeypoints: [463, 398, 384, 385, 386, 387, 388, 466, 263, 255],
      eyeBottomArcKeypoints: [463, 341, 256, 252, 253, 254, 339, 255],
    },
  ].map(({ eyeTopArcKeypoints, eyeBottomArcKeypoints }) => {
    const topLeftOrigin = {
      x: Math.round(
        Math.min(...eyeTopArcKeypoints.map((k) => scaledMesh[k][0]))
      ),
      y: Math.round(
        Math.min(...eyeTopArcKeypoints.map((k) => scaledMesh[k][1]))
      ),
    };
    const bottomRightOrigin = {
      x: Math.round(
        Math.max(...eyeBottomArcKeypoints.map((k) => scaledMesh[k][0]))
      ),
      y: Math.round(
        Math.max(...eyeBottomArcKeypoints.map((k) => scaledMesh[k][1]))
      ),
    };
